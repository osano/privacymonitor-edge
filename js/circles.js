(function(root, factory) {
  ...
  Circles = function(options) {
    ...
    this._previusScore   = options.previousScore;
  ...
  Circles.prototype = {
    ...
    _generatePath: function(percentage, open, color, pathClass) {
      ...
      path.setAttribute("stroke-dasharray", this._previusScores(this._previusScore));
      this._svg.appendChild(path);

      return this;
    },

    _previusScores: function(previusScore) {
      if (previusScore == false) {
        return 999
      }else {
        let previusScoreXYZ = previusScore / 4.25 - 6;
         previusScoreXYZ = Math.round(previusScoreXYZ);

         return previusScoreXYZ + " 2.5 200"
      }
    ...
    updatePreviusScore: function(prevScore) {
      this._previusScore = prevScore;
